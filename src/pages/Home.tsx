...
const Home = () => {
  ...
  return <div className="min-h-screen">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        {/* Carousel Controls */}
        <Button variant="ghost" size="icon" onClick={prevSlide} className="absolute left-4 top-1/2 transform -translate-y-1/2 text-white hover:bg-white/20 z-10">
          <ChevronLeft className="w-6 h-6" />
        </Button>
        <Button variant="ghost" size="icon" onClick={nextSlide} className="absolute right-4 top-1/2 transform -translate-y-1/2 text-white hover:bg-white/20 z-10">
          <ChevronRight className="w-6 h-6" />
        </Button>

        {/* Language Toggle */}
        <Button variant="ghost" size="sm" onClick={toggleLanguage} className="absolute top-4 right-4 z-20 text-white hover:bg-white/20 border border-white/30">
        ...
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-2 z-10">
          {carouselImages.map((_, index) => <button key={index} onClick={() => setCurrentSlide(index)} className={`w-3 h-3 rounded-full transition-all ${index === currentSlide ? "bg-white" : "bg-white/50"}`} />)}
        </div>
      </section>
      ...
      <section className="py-20 px-4 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, index) => <NavLink key={index} to={feature.link}>
      ...
      <section className="py-20 gradient-sunset">
        <div className="max-w-4xl mx-auto text-center px-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <NavLink to="/gallery">
              <Button size="lg" variant="outline" className="font-semibold px-8 border-white hover:bg-white text-slate-800">
